...
def AD(acad, center, radius, start_angle, end_angle, leader_length=20, 
       upper_tolerance=0.0, lower_tolerance=0.0, layer="轮廓线", dim_style="ZqStandard", chord_angle=None):
    """绘制圆弧并创建标注
    ...
    try:
        ...
        if chord_angle is None:
            chord_angle = (start_angle + end_angle) / 2
        
        # 计算弦点坐标
        chord_point = APoint(
            center.x + abs_radius * math.cos(chord_angle),
            center.y + abs_radius * math.sin(chord_angle)
        )
        
     
        dim = acad.model.AddDimRadial(center, chord_point, leader_length)
        dim.Layer = "标注线"
...
def demo():
    ...
    try:
        ...
        chord_angle = math.radians(90)  # 指定弦点角度为90度
        arc4, dim4 = AD(acad, center4, radius, start_angle, end_angle, 70, 0.02, 0.02, chord_angle=chord_angle)
        ...
        if all([arc1, dim1, arc2, dim2, arc3, dim3, arc4, dim4, arc5, dim5]):
            ...
            print(f"示例4 - 样式: {dim4.StyleName}, 长度: {dim4.TextOverride}, 对称公差: ±{dim4.ToleranceUpperLimit}, 弦点角度: {math.degrees(chord_angle):.0f}°")
